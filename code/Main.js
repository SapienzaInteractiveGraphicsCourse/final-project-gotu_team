...
//  *** Meshes ***
	...
	collidableMeshList.push(wall_6);
...
// **** FLAGs and Game VARs **** 
	...
	switch (level){
		case "LEVEL 2": {
			scene.add(wall_5); collidableMeshList.push(wall_5);
		}
		case "LEVEL 3": {
			scene.add(wall_5); collidableMeshList.push(wall_5);
...
// **** FUNCTIONS ****
    ...
    var RESTART = function(){
		...
		rockSphere.position.x = 0; rockSphere.position.y = 0;
		currentLane = 0.0;
...
var popObstacles = function(obj){
	...
	if (obj.position.y < -10){ // y < -10
        rotateOnPivot(obj, groundCenter, rotAxis, -1); // -50
...
var generateNewObstacle = function(obj){
	...
	randomY = THREE.MathUtils.randInt(-1 , 0)
	if (obsDistance==1) randomX=randomX*2;
	// console.log(randomX)

	obj.position.x = randomX;
	// obj.position.y = randomY;
}
...
var checkCollisions = function(obj){
	if (collisionTime > 0){
		...
		if (collisionTime%2==1) rockSphere.visible = false;
...
var rockSpinning = function(){
    
    rockSphere.rotation.x -= rockRollingSpeed;   // Rock rotation
